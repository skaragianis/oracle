...
Persistent state (the SQLite database, the uploads folder, and the vector index) is one dataset and lives together on a single volume: the image sets `ORACLE_DB_PATH=/data/oracle.db`, `ORACLE_UPLOADS_DIR=/data/uploads`, and `ORACLE_VECTOR_DB_PATH=/data/vectors`, and `make up` mounts the `oracle-data` named volume at `/data`. The embedding model cache (`ORACLE_MODEL_CACHE_DIR=/data/models`) sits on the same volume so the startup download happens once per volume, not once per container. `make up` is rerunnable — it rebuilds and replaces the container without touching the volume. Keep the env vars pointing at the same place; rows without their files or vectors (or vice versa) are corrupt state.
