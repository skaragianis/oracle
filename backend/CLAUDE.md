...
- **Search is hybrid.** `search.search_hybrid()` returns the top 10 BM25 (FTS5) results followed by the top 10 vector results, each `SearchResult` tagged with `source` (`"bm25"`/`"vector"`); merging happens here, never in a client. Vector search returns the *nearest* chunks unconditionally — there is no similarity threshold, so a query that matches nothing by keyword still returns vector results once anything is indexed.
