...
- **A document is only searchable once its status is `ready`.** Uploading returns `202` with the document `pending`: the backend chunks it in a background task, after which it settles on `ready` or `failed`. `.docx` currently always ends `failed` ("cannot chunk .docx yet"), because nothing chunks it. Unready rows are greyed out and cannot be selected — don't treat `pending` as selectable. A `pending` document needs polling (`GET /documents`) to learn its outcome; the upload response never carries it.
...
- **Search is hybrid and merged server-side**: one list of top-10 BM25 plus top-10 vector results, each carrying `source` (`'bm25' | 'vector'`). The frontend only renders the tag — never re-merge, re-rank, or dedupe here. The same chunk can appear once per source, so list keys must include `source`, not just `chunk_id`. A query with no keyword match still returns vector results (nearest neighbours, no threshold) — an empty result list only happens when nothing is indexed.
