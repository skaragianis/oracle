...
- **Search runs against selectable sources, fused server-side when both are chosen** (reciprocal rank fusion over top-10 BM25 + top-10 vector, capped at 5 results): each result carries `sources`, the list of indexes that returned it (`'bm25'`, `'vector'`, or both). `SearchPanel` sends the checked sources (`bm25`, `vector`, or both — never neither, since that disables the Ask button) via `search(query, sources)`. The frontend only renders one tag per source — never re-merge, re-rank, or dedupe here. Fusion means `chunk_id` is unique per result and safe as a list key. A query with no keyword match still returns vector-sourced results (nearest neighbours, no threshold) when vector is selected, so an empty result list only guarantees nothing is indexed when both sources are checked — BM25-only legitimately returns "No matches found" for a query with no keyword hits.
